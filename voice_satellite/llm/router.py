"""
LLMRouter — Async OpenClaw and Conversational Coordinator LLM Router.
"""
...
logger = logging.getLogger("voice_satellite.llm")
...
class LLMRouter:
    """
    Coordinator router that decides whether to answer the user directly,
    delegate to an OpenClaw agent, check background tasks, or end the conversation.
    """
    def __init__(
        self,
        ...
        if not api_key:
            raise ValueError("LITELLM_API_KEY is required for LLM coordinator routing")
    ...
    async def decide(
        ...
        """
        Routes the user's transcript to a coordinator decision.
        """
    ...
    async def route(
        ...
        """
        Wraps decide() method to match user prompt requirement:
        route(transcript, history, task_summaries) -> CoordinatorDecision
        """
        if history is not None:
            self.history = list(history)
        return await self.decide(transcript, task_context=task_summaries or "")
    ...
    async def complete_with_tool_result_stream(
        ...
        """
        Streams a natural response answering the user based on OpenClaw tool output.
        """
    ...
    def remember_turn(self, user_message: str, assistant_message: str) -> None:
        """
        Saves the last conversation turn into history.
        """
