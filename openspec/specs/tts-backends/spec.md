...
Swappable TTS backends behind one protocol: a plain registry selects the voice
by name from config, Piper is the zero-setup default so a fresh install speaks
with no TTS configuration, and Genie/GPT-SoVITS remains the custom
voice-cloning path. Introduced by the `pluggable-tts` change (2026-07-08).
