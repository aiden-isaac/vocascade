# skill-sdk Specification

## Purpose
The contract every skill (bundled or user) can rely on: streaming handlers,
`ctx.task_broker` async dispatch, `ctx.notify` proactive delivery, the
`available` gate, and the self-contained reference agent skill.
## Requirements
