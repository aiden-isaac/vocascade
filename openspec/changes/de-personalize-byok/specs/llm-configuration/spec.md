...
How the fast-brain (OpenAI-compatible LLM) and heavy-brain (Hermes agent)
endpoints are configured: required vs. optional values, absence of personal
defaults, fail-fast behavior, and local-only mode when Hermes is absent.
