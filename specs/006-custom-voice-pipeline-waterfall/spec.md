...
3. **Given** an utterance no skill claims, **When** routing reaches the fallback, **Then** the bundled smalltalk skill (fixed low confidence) handles it.
4. **Given** a new skill is registered with example phrases, **When** the app starts, **Then** the medium-stage classifier prompt is regenerated to include those examples with no manual prompt editing.
...
- **FR-030**: A bundled smalltalk skill MUST act as a fixed low-confidence fallback that wins only when nothing above it scored higher.
- **FR-031**: Smalltalk replies MUST be generated by the local LLM in the assistant's configured persona.
- **FR-032**: The local LLM's responsibilities MUST be limited to smalltalk replies, the medium-stage classifier, and optimistic partial openings; it MUST NOT be given the Hermes/skill tool schemas.
